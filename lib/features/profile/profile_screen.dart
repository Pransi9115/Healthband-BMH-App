...
class _ProfileScreenState extends State<ProfileScreen>
  ...
  // What Health Connect has from OTHER apps (Google Fit, Samsung Health,
  // the phone's own step counter, etc.) — informational only, shown
  // after a sync. Never merged into BMH's own live readings.
  Map<String, String> _fromOtherApps = {};
  ...
  // now — so "Connected" survives app restarts instead of resetting to
  // false every time, and so a permission revoked in Android Settings
  // is correctly reflected here too.
  Future<void> _checkHealthConnectState() async {
    final granted = await HealthService.hasHealthConnectPermissions();
  ...
  Widget _buildHealthCard() {
    ...
    return Container(
      ...
      child: Column(children: [
        ...
        // ── FROM OTHER APPS ────────────────────────────
        // What Health Connect already has from sources other than BMH —
        // shown after a sync, informational only. Not merged into BMH's
        // own live band readings anywhere.
        if (_healthConnected && _fromOtherApps.isNotEmpty)
          Padding(
            padding: const EdgeInsets.fromLTRB(18, 0, 18, 18),
            child: Container(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text('Also found in Health Connect',
                    style: BMHText.monoSm.copyWith(
                      color: BMHColors.inkMute, fontSize: 9)),
                  const SizedBox(height: 8),
                  ..._fromOtherApps.entries.map((e) => Padding(
                    padding: const EdgeInsets.only(bottom: 4),
                    child: Row(children: [
                      Expanded(child: Text(e.key,
                        style: BMHText.bodySm.copyWith(
                          color: BMHColors.inkDim, fontSize: 11))),
                      Text(e.value, style: BMHText.monoSm.copyWith(
  ...
  Future<void> _connectGoogleHealth() async {
    // Android only — Health Connect doesn't exist on iOS.
    if (!Platform.isAndroid) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text('Google Health Connect is Android-only.',
    ...
    if (!available) {
      ...
      final shouldInstall = await showDialog<bool>(
        ...
        builder: (ctx) => AlertDialog(
          ...
          title: Text('Health Connect needed', style: BMHText.heading2),
          content: Text(
            'BMH uses the Health Connect app to share data with other '
            'health apps. It looks like it isn\'t installed (or needs '
            'an update) on this phone.',
    ...
    final result = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Column(mainAxisSize: MainAxisSize.min, children: [
          Text('Android will ask you to confirm which data to share with '
               'Health Connect — BMH will request all of these:',
            style: BMHText.bodySm.copyWith(color: BMHColors.inkDim)),
          const SizedBox(height: 16),
          ...['❤️ Heart Rate', '👣 Steps', '🩸 SpO2',
            '💉 Blood Pressure', '😴 Sleep', '🌡️ Temperature',
            '🧬 HRV', '🔥 Calories', '📍 Distance']
            .map((d) => Padding(
    ...
    // This hands off to Android's own native permission screen — the
    // user decides what to actually grant there, not in this app.
    final granted = await HealthService.requestHealthConnectPermissions();
    if (!mounted) return;
    ...
    if (granted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text('Permission wasn\'t granted in Health Connect.',
          style: BMHText.monoSm.copyWith(color: BMHColors.bg0)),
        backgroundColor: BMHColors.danger,
        behavior: SnackBarBehavior.floating));
    }
  ...
  final result = await HealthService.syncWithHealthConnect(
    ...
    hrv: _ble.hrv.toDouble(),
    calories: _ble.calories,
    distanceKm: _ble.distance,
    steps: _ble.steps,
    sleepMinutes: _ble.lastSleep?.totalMinutes ?? 0,
  );
  ...
  setState(() {
    ...
    _healthSyncDone = result.ok;
    _fromOtherApps = result.fromOtherApps;
    if (result.ok) _lastSynced = DateTime.now();
  });

  if (!result.ok) {
    // ENHANCED: Show more detailed error messages
    String displayError = result.error ?? 'Sync failed — try again.';
    String snackBarMessage = displayError;
    
    // Add helpful suggestions based on error type
    if (result.error?.contains('Permission') == true) {
      snackBarMessage = 'Permissions missing. Tap to fix in Settings.';
    } else if (result.error?.contains('timeout') == true ||
               result.error?.contains('network') == true) {
      snackBarMessage = 'Network error. Check your connection.';
    } else if (result.error?.contains('Android') == true) {
      snackBarMessage = 'Android-only feature. Use a different device.';
    }
    ...
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
      ...
      action: result.error?.contains('Permission') == true
        ? SnackBarAction(
            label: 'Fix',
            textColor: Colors.white,
            onPressed: () {
              // Could open settings or re-request permissions here
              HealthService.requestHealthConnectPermissions();
            })
        : null,
    ));
    
    // ENHANCED: Log detailed debug info for troubleshooting
    if (result.debugInfo != null) {
      debugPrint('[ProfileScreen] Sync error details:');
      debugPrint('[ProfileScreen] Error: ${result.error}');
      debugPrint('[ProfileScreen] Debug Info: ${result.debugInfo}');
    }
  } else {
    ...
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
      content: Row(children: [
        ...
        const SizedBox(width: 8),
        Expanded(child: Text(
          'Synced! ${result.written} metrics written.',
          style: BMHText.monoSm.copyWith(color: BMHColors.bg0))),
      ]),
      backgroundColor: BMHColors.sGut,
      behavior: SnackBarBehavior.floating,
      duration: const Duration(seconds: 3),
    ));
  }
