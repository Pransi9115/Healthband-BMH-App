...
class _BioScoreCard extends StatelessWidget {
  ...
  int get _score {
    // Only calculate if band is connected AND being worn
    if (!ble.isBandConnected || !ble.isWearing) return 0;
    int s = 70;
    final hr = ble.heartRate; final spo2 = ble.spo2; final hrv = ble.hrv;
    if (hr > 0) { if (hr >= 55 && hr <= 70) s += 8; else if (hr > 70 && hr <= 80) s += 4; else if (hr > 100) s -= 8; }
    if (spo2 > 0) { if (spo2 >= 98) s += 6; else if (spo2 < 94) s -= 10; }
    if (hrv > 0) { if (hrv >= 40) s += 6; else if (hrv >= 25) s += 3; }
    return s.clamp(40, 100);
  }

  bool get _hasScore => ble.isBandConnected && ble.isWearing && ble.heartRate > 0;

  String get _label { if (_score >= 85) return 'Excellent'; if (_score >= 70) return 'Good'; if (_score >= 55) return 'Fair'; return 'Needs attention'; }
      ...
      Text.rich(TextSpan(children: [
        TextSpan(text: value,
          style: BMHText.displaySm.copyWith(
            ...
            fontFamily: 'JetBrains Mono', height: 1.1)),
        if (unit.isNotEmpty)
          TextSpan(text: ' $unit',
            style: BMHText.monoSm.copyWith(
              fontSize: 8, color: BMHColors.inkMute)),
